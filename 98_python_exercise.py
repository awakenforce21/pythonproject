...
while pre1 < 4000000:
    now = pre1 + pre2
    if now % 2 == 0 and now < 4000000:
        sum2 += now
    pre1 = pre2
    pre2 = now

print("2번 답: {}".format(sum2))
...
q_str = "This is a sample Program mississippi river."
q_str_1 = "abcdabcdababccddcd"
check = dict()
...
maxVal = 0
maxWord = ' '
for tmp in check.keys():
    if check[tmp] > maxVal:
        maxWord = tmp
        maxVal = check[tmp]
    elif check[tmp] == maxVal and maxVal != 0:
        if maxWord > tmp:
           maxWord = tmp
           maxVal = check[tmp]
print("3번 답: {}".format(maxWord))
## 문제 4.
## 로또 프로그램 작성
...
def get6num(my_list):
    for i in range (6):
        my_list.append(random.randint(1, 46))
...
rst_dict = {6:0, 5:0, 4:0, 3:0, 2:0, 1:0, 0:0}

for i in range(5):
    temp = []
    get6num(temp)
    rst = same_count(lucky, temp)
    rst_dict[rst] += 1

print("Easy - 1등: {}개 / 2등: {}개 / 3등: {}개 / 4등: {}개 / 꽝: {}개"
    .format(rst_dict[6], rst_dict[5], rst_dict[4], rst_dict[3], rst_dict[2]+rst_dict[1]+rst_dict[0]))
